use hellas_morpheus::{
    ...
    Signature, Signed, SlotNum, StartView, ThreshSigned, ThreshSignature, Transaction, 
    ViewNum, VoteData,
};
use std::sync::Arc;
...
fn test_format_functions() {
    ...
    let messages = vec![
        ...
        Message::NewVote(signed_vote.clone()),
    ...
    println!("BlockType::Genesis: {}", format_block_type(&BlockType::Genesis));
    ...
    println!("Signed<VoteData>: {}", format_signed(&signed_vote, |vd| format_vote_data(vd, false), false));
    println!("ThreshSigned<VoteData>: {}", format_thresh_signed(&thresh_signed_vote, |vd| format_vote_data(vd, false), false));
    
    println!("\n==== Block Types ====");
    println!("Block: {}", format_block(&block, false));
    println!("Signed<Block>: {}", format_signed(&signed_block, |b| format_block(b, false), false));
