...
impl<Tr: Transaction> ProcessState<Tr> {
    ...
    pub fn new(genesis_block: Arc<Signed<Block<Tr>>>, genesis_qc: FinishedQC) -> Self {
    ...
    pub fn apply(&mut self, effect: &Effect<Tr>, id: &Identity) {
        match effect {
        ...
        match effect {
            ...
            Effect::BlockProduced { block } | Effect::BlockRecorded { block } => {
            ...
            Effect::QcRecorded {
                ...
                self.insert_qc(qc, id);
            }

            Effect::VoteSent { vote, target: _ } => {
                if let Some(author) = &vote.data.for_which.author {
                    self.voted.insert((
                        vote.data.z,
                        vote.data.for_which.type_,
                        vote.data.for_which.slot,
                        author.clone(),
                    ));
                }
            }
            ...
            Effect::VoteRecorded { voter, vote } => {
                let votes = self.vote_tracker.entry(vote.data.clone()).or_default();
            ...
            Effect::ViewChanged { old_view: _, new_view } => {
                self.current_view = *new_view;
                self.current_phase = Phase::High; // Views always start in high phase
                self.phase_by_view.insert(*new_view, Phase::High);
                self.view_entry_time = self.current_time;
                self.produced_lead_in_view.insert(*new_view, false);
                
                // Mark pending votes as dirty for the new view
                self.pending_votes.entry(*new_view).or_default().dirty = true;
            }

            Effect::EndViewRecorded { voter, view, vote } => {
                self.end_views
                    .entry(*view)
                    .or_default()
            ...
            Effect::ViewCertFormed { cert: _ } => {
                // Certificate formation is tracked through MessageSent
            }

            Effect::StartViewSent { view, tips, .. } => {
                // Track that we sent our tips for this view
                for tip in tips {
                    self.complained_qcs.insert(tip.clone());
                }
            }
            ...
            Effect::PendingVoteProcessed {
                ...
                if let Some(pending) = self.pending_votes.get_mut(view) {
                    match (*vote_type, block_type) {
                        (1, BlockType::Tr) => {
                            pending.tr_1.remove(block_key);
                        }
                        (2, BlockType::Tr) => {
                            pending.tr_2.remove(block_key);
                        }
                        (1, BlockType::Lead) => {
                            pending.lead_1.remove(block_key);
                        }
                        (2, BlockType::Lead) => {
                            pending.lead_2.remove(block_key);
                        }
                        _ => {}
                    }
                }
    ...
    fn insert_block(&mut self, block: &Arc<Signed<Block<Tr>>>) {
        ...
        if self.blocks.contains_key(&key) {
            tracing::error!("Block already exists: {:?}", key);
            return;
        }
        ...
        if key.height > self.max_height.0 {
            tracing::debug!(
                target: "new_max_height",
                prev_height = self.max_height.0,
                new_height = key.height,
            );
        ...
        match key.type_ {
            ...
            BlockType::Tr => {
                pending.tr_1.insert(key.clone(), true);
            }
            _ => {}
        }
    }
    ...
    fn insert_qc(&mut self, qc: &FinishedQC, process_id: &Identity) {
        if !self.qcs.insert(qc.clone()) {
            tracing::error!("QC already exists: {:?}", qc.data.for_which);
            return;
        }
        ...
        if qc.data.z == 1 && qc.data.compare_qc(&self.max_1qc.data) == std::cmp::Ordering::Greater {
            ...
            tracing::debug!(
                target: "new_max_1qc",
                prev_1qc = ?self.max_1qc.data.for_which,
                new_1qc = ?qc.data.for_which,
            );
        }
        ...
        // Only add to unfinalized_2qc AFTER checking finalization
        if qc.data.z == 2 {
            self.unfinalized_2qc.insert(qc.clone());
        }

        // Update tips
        self.update_tips_for_qc(qc);
    ...
    fn update_tips_for_qc(&mut self, qc: &FinishedQC) {
        ...
        for tip in &self.tips {
            if self.observes(&qc.data, &tip.data) {
                tracing::debug!(
                    target: "yeet_tip",
                    new_tip = ?qc.data.for_which,
                    old_tip = ?tip.data.for_which,
                );
        ...
        if !tips_to_remove.is_empty() {
            ...
            tracing::debug!(
                target: "new_tip",
                reason = "extends existing tip",
                num_displaced = tips_to_remove.len(),
                qc = ?qc.data);
