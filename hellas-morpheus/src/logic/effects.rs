...
pub enum Effect<Tr: Transaction> {
    ...
    /// Complaint sent
    ComplaintSent { qc: FinishedQC, target: Identity },

    /// End-view message sent
    EndViewSent { view: ViewNum },

    /// View certificate formed
    ViewCertificateFormed { view: ViewNum },
    ...
    pub fn description(&self) -> String {
        match self {
            ...
            Effect::VoteSent {
                ...
                format!(
                    "Sent {}-vote for {:?} to {}",
                    vote_type, block_key, target_str
                )
            }
            Effect::VoteRecorded { voter, vote_data } => {
                format!(
                    "Recorded {}-vote from {} for {:?}",
                    vote_data.z, voter.0, vote_data.for_which
                )
            }
            Effect::QuorumReached { vote_data, .. } => {
                format!(
                    "Quorum reached for {}-vote on {:?}",
                    vote_data.z, vote_data.for_which
                )
            }
