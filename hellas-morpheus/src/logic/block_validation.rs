...
impl fmt::Display for BlockValidationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::LeaderMissingPrevPointers { key } => {
                write!(
                    f,
                    "Leader block with key {:?} is missing predecessor pointers",
                    key
                )
            }
            Self::InsufficientJustification {
                ...
                write!(f, "Leader block with key {:?} has insufficient justification. Found {}, required {}", key, found, required)
            }
            Self::NotEnoughJustifications => write!(
                f,
                "Leader block justification has insufficient number of justifications"
            ),
        }
    }
}

pub fn block_valid<Tr: Transaction>(
    kb: &KeyBook,
    n: u32,
    f: u32,
    ...
    if !signed_block.valid_signature(kb) {
    ...
    let max_prev_height = block
        .prev
        .iter()
        .map(|qc| qc.data.for_which.height)
        .max()
        .unwrap_or(0);
    if block.key.height != max_prev_height + 1 {
    ...
    // Check prev QCs views and heights
    for qc in &block.prev {
        if qc.data.for_which.view > block.key.view {
            return Err(BlockValidationError::PrevQcViewGreaterThanBlockView {
                prev_view: qc.data.for_which.view,
                block_view: block.key.view,
            });
        }
        if qc.data.for_which.height >= block.key.height {
            return Err(
                BlockValidationError::PrevQcHeightGreaterOrEqualBlockHeight {
                    prev_height: qc.data.for_which.height,
                    block_height: block.key.height,
                },
            );
        }
        if qc != genesis_qc && !qc.valid_signature(kb, n - f) {
    ...
    if block.one.data.for_which.type_ != BlockType::Genesis {
        if !block.one.valid_signature(kb, n - f) {
            return Err(BlockValidationError::InvalidOneQcSignature);
        }
    } else if genesis_qc != &block.one {
        return Err(BlockValidationError::InvalidGenesisOneQc);
    }

    // Type-specific validation
    match (&block.key.type_, &block.data) {
        (BlockType::Tr, BlockData::Tr { transactions }) => {
            if transactions.is_empty() {
                return Err(BlockValidationError::EmptyTransactions);
            }
            if !block.key.slot.is_zero() && !block.prev.iter().any(|qc| {
        ...
        (BlockType::Lead, BlockData::Lead { justification }) => {
            // Leader blocks must be authored by the leader of the view
            let expected_leader = Identity((block.key.view.0 as u32 % n) + 1);
            ...
            if !block.key.slot.is_zero() {
                ...
                if prev_leader_for[0].data.for_which.view == block.key.view && block.one.data.for_which != prev_leader_for[0].data.for_which {
            ...
            if block.key.slot.is_zero() || prev_leader_for[0].data.for_which.view < block.key.view {
                if justification.len() < (n - f) as usize {
                    return Err(BlockValidationError::InvalidJustificationSize {
                        size: justification.len() as u64,
                        expected: (n - f) as u64,
                    });
                }

                if !justification.iter().all(|j| j.valid_signature(kb)) {
        ...
        (block_type, _) => Err(BlockValidationError::BlockDataTypeMismatch {
            key_type: *block_type,
            data_type: match &block.data {
                BlockData::Genesis => BlockType::Genesis,
                BlockData::Tr { .. } => BlockType::Tr,
                BlockData::Lead { .. } => BlockType::Lead,
            },
        }),
    }
}
