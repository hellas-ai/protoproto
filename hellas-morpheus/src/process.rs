...
pub struct MorpheusProcess {
    /// Cryptographic key material for signature verification and creation
    pub kb: KeyBook,
    ...
    /// Core DAG structure and QC tracking
    ...
    /// The genesis block that starts the DAG
    pub genesis: Arc<Signed<Block>>,
    
    /// The genesis block's quorum certificate
    ...
    /// Votes pending for reevaluation in each view
    pub pending_votes: BTreeMap<ViewNum, PendingVotes>,
}

impl MorpheusProcess {
    /// Creates a new MorpheusProcess with the given parameters
    ///
    /// Initializes all protocol state variables to their initial values
    /// and creates the genesis block and its QC.
    pub fn new(keybook: KeyBook, id: Identity, n: usize, f: usize) -> Self {
        ...
        MorpheusProcess {
            ...
            // Auxiliary fields
            end_views: QuorumTrack::new(),
            zero_qcs_sent: BTreeSet::new(),
            complained_qcs: BTreeSet::new(),
            view_entry_time: 0,
            current_time: 0,

            vote_tracker: QuorumTrack::new(),
