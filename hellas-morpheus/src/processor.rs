...
impl<Tr: Transaction> ActionProcessor<Tr> {
    pub fn new(id: Identity, n: u32, f: u32, delta: u128, kb: KeyBook) -> Self {
        Self { id, n, f, delta, kb, _phantom: PhantomData }
    }
    ...
    pub fn process_action(
        ...
        match action {
            ...
            Action::CheckTimeouts => {
                self.check_timeouts(state)
            }
            Action::CheckProduceBlocks => {
                self.check_produce_blocks(state)
            }
        }
    }
    ...
    fn process_block(
        ...
        if let Err(_) = self.validate_block(block, state) {
    ...
    fn check_pending_votes(&self, state: &dyn ProcessState<Tr>) -> Vec<Effect<Tr>> {
        ...
        if state.contains_lead_in_view(current_view) && state.has_unfinalized_lead_in_view(current_view) {
            // Check for eligible 1-votes on transaction blocks
            for (block_key, block) in self.get_pending_tr_blocks(state, current_view) {
        ...
        if current_phase == Phase::High {
            ...
            for (block_key, block) in self.get_pending_lead_blocks(state, current_view) {
    ...
    fn validate_block(&self, block: &Arc<Signed<Block<Tr>>>, state: &dyn ProcessState<Tr>) -> Result<(), String> {
        ...
        impl<'a> ValidationContext<'a> {
            fn block_valid<Tr: Transaction>(&self, signed_block: &Signed<Block<Tr>>) -> Result<(), crate::block_validation::BlockValidationError> {
    ...
    fn form_qc_from_votes(&self, vote_data: &VoteData, votes: &[Arc<ThreshPartial<VoteData>>]) -> FinishedQC {
        ...
        if vote_sigs.len() < (self.n - self.f) as usize {
            panic!("Not enough votes to form QC: {} < {}", vote_sigs.len(), self.n - self.f);
        }
        
        // Aggregate the signatures
        let agg = self.kb.hints_setup.as_ref()
    ...
    fn form_view_certificate(&self, view: &ViewNum, state: &dyn ProcessState<Tr>) -> Arc<ThreshSigned<ViewNum>> {
    ...
    fn find_finalized_blocks(&self, qc: &FinishedQC, state: &dyn ProcessState<Tr>) -> Vec<BlockKey> {
    ...
    fn produce_tr_block(&self, state: &dyn ProcessState<Tr>) -> (Arc<Signed<Block<Tr>>>, Vec<Effect<Tr>>) {
        ...
        if state.tips_count() == 1 {
            ...
            if !prev_qcs.iter().any(|qc| qc.data.for_which == tip.data.for_which) {
    ...
    fn produce_lead_block(&self, state: &dyn ProcessState<Tr>) -> (Arc<Signed<Block<Tr>>>, Vec<Effect<Tr>>) {
        ...
        if !slot.is_zero() {
            if let Some(prev_qc) = state.latest_leader_qc() {
                if prev_qc.data.for_which.slot.is_pred(slot) {
                    if !prev_qcs.iter().any(|qc| qc.data.for_which == prev_qc.data.for_which) {
        ...
        let (one_qc, justification) = if !has_produced {
            let view_messages = state.get_start_views(view)
                .map(|msgs| msgs.clone())
                .unwrap_or_default();
            ...
            let prev_qc = state.latest_leader_1qc()
                .cloned()
                .unwrap_or_else(|| state.max_1qc().clone());
            (prev_qc, vec![])
        };
        ...
        let effects = vec![
            ...
            Effect::LeaderBlockProducedInView {
                view,
            },
        ];
    ...
    fn get_pending_tr_blocks(&self, state: &dyn ProcessState<Tr>, view: ViewNum) -> Vec<(BlockKey, Arc<Signed<Block<Tr>>>)> {
    ...
    fn get_pending_lead_blocks(&self, state: &dyn ProcessState<Tr>, view: ViewNum) -> Vec<(BlockKey, Arc<Signed<Block<Tr>>>)> {
    ...
    fn get_pending_lead_1qcs(&self, state: &dyn ProcessState<Tr>, view: ViewNum) -> Vec<FinishedQC> {
...
pub trait ProcessState<Tr: Transaction> {
    ...
    fn get_unvoted_blocks(&self, view: ViewNum, vote_type: u8, block_type: BlockType) -> Vec<BlockKey>;
