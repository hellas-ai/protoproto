/*
 * Debug Formatting Utilities for Morpheus Protocol Types
 * 
 * This module provides utility functions for formatting Morpheus protocol types
 * in a concise and human-readable way for logging and debugging purposes.
 * 
 * Instead of relying on Debug trait implementations, which can be verbose and hard
 * to read, these utilities provide custom formatting that focuses on the most
 * important information for each type.
 * 
 * Example usage:
 * 
 * ```
 ...
 * protocol_log!("Processing view change to {}", format_view_num(&view_num));
 * block_log!(&block);
 * vote_log!(&vote_data);
 * qc_log!(&thresh_signed_vote);
 * message_log!(&message);
 * ```
 * 
 * The format functions follow a consistent pattern:
 * - Compact output by default (verbose=false)
 * - Detailed output when verbose=true
 * - Clear type identification and clear field separation
 */
...
macro_rules! block_log {
    ($block:expr) => {
        println!("[BLOCK] {}", $crate::debug_impls::format_block($block, false))
    };
    ($block:expr, true) => {
        println!("[BLOCK] {}", $crate::debug_impls::format_block($block, true))
    };
}
...
macro_rules! vote_log {
    ($vote:expr) => {
        println!("[VOTE] {}", $crate::debug_impls::format_vote_data($vote, false))
    };
}
...
macro_rules! qc_log {
    ($qc:expr) => {
        println!("[QC] {}", $crate::debug_impls::format_thresh_signed(
            $qc, 
            |vd| $crate::debug_impls::format_vote_data(vd, false), 
            false
        ))
    };
}
...
macro_rules! message_log {
    ($msg:expr) => {
        println!("[MSG] {}", $crate::debug_impls::format_message($msg, false))
    };
    ($msg:expr, true) => {
        println!("[MSG] {}", $crate::debug_impls::format_message($msg, true))
    };
} 