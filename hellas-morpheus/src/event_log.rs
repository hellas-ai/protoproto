...
fn default_log_table<Tr: Transaction>() -> Option<TableDefinition<'static, u64, Postcard<LogEntry<Tr>>>> {
    Some(TableDefinition::new("event_log"))
}

pub fn default_snapshots_table<Tr: Transaction>() -> Option<TableDefinition<'static, u64, Postcard<MorpheusProcess<Tr>>>> {
...
impl<Tr: Transaction> EventLog<Tr> {
    pub fn new(db: &Database) -> Self {
        ...
        Self {
            ...
            snapshots_table: default_snapshots_table::<Tr>(),
        }
    }
    ...
    pub fn save_snapshot(
        ...
        {
            let mut tbl = tx.open_table(self.snapshots_table.unwrap())
                .map_err(|e| format!("Failed to open snapshots table: {:?}", e))?;
            
            tbl.insert(snapshot_index, process)
    ...
    pub fn load_snapshot_before(
        &self,
        db: &Database,
        before_index: u64,
    ) -> Result<Option<(u64, MorpheusProcess<Tr>)>, String> {
        let tx = db.begin_read().unwrap();
        let tbl = tx.open_table(self.snapshots_table.unwrap())
            .map_err(|e| format!("Failed to open snapshots table: {:?}", e))?;
            ...
            match result {
                Ok((index, snapshot)) => {
                    if index.value() < before_index {
                        latest_snapshot = Some((index.value(), snapshot.value()));
                    } else {
                        break;
                    }
                }
        ...
        // Initialize the processor if we found a snapshot
        if let Some((index, mut process)) = latest_snapshot {
            process.init_processor();
    ...
    pub fn cleanup_before(
        ...
        {
            let mut log_tbl = tx.open_table(self.log_table.unwrap())
                .map_err(|e| format!("Failed to open log table: {:?}", e))?;
            
            let mut to_remove = Vec::new();
            let range = log_tbl.range(..keep_after)
        ...
        {
            let mut snapshots_tbl = tx.open_table(self.snapshots_table.unwrap())
                .map_err(|e| format!("Failed to open snapshots table: {:?}", e))?;
            
            let mut to_remove = Vec::new();
            let range = snapshots_tbl.range(..keep_after)
                .map_err(|e| format!("Failed to create range: {:?}", e))?;
